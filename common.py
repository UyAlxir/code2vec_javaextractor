...
class common:
    ...
    @staticmethod
    def parse_prediction_results(raw_prediction_results, unhash_dict, hash_path_to_score_dict, special_words, topk: int = 5) -> List['MethodPredictionResults']:
        prediction_results = []
        for single_method_prediction in raw_prediction_results:
            ...
            # 猜测key是路径，attention_per_context是一个数组？（或许是映射）用来得到key的attention值
            ...
            for context, attention in topk_attention_per_context:
                ...
                if hashed_path in unhash_dict:
                    ...
                    score_list = hash_path_to_score_dict[hashed_path]
                    # print(unhashed_path)
                    # print(score_list)
                    # attention <-> path at here
                    # attention.item() 是分数值
                    attention_value = attention.item()
                    line_cnt = score_list.split('_')
                    # print(line_cnt)
                    for single_line_cnt in line_cnt:
                        single_couple = single_line_cnt.replace('(', '')
                        single_couple = single_couple.replace(')', '')
                        single_couple = single_couple.split('^')
                        # print(single_couple)
                        if(len(single_couple) > 1):
                            line = single_couple[0]
                            cnt = single_couple[1]
                            if line in line_score_map.keys():
                                line_score_map[line] += float(attention_value)
                            else:
                                line_score_map[line] = float(attention_value)

                    current_method_prediction_results.append_attention_path(attention.item(), token1=token1,
                                                                            path=unhashed_path, token2=token2)
                    # print(current_method_prediction_results.attention_paths)
            # print(line_score_map)
            topk_attention_line = sorted(line_score_map.items(), key=lambda x: x[1], reverse=True)
            # print(topk_attention_line)
            print("\n\n======================the topk line by attention======================")
            print("the origianl method name is:" + str(current_method_prediction_results.original_name))
            for single_line in topk_attention_line[:topk]:
                print('line number: %s\t\t\t attention value:%s' % (single_line[0], str(single_line[1])))
            print("======================================================================\n\n")
